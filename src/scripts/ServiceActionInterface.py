...
class ServiceActionInterface(BaseActionInterface):
    ...
    def run_thread(self, dispatch_msg):
        ...
        if self._action_config['name'] in ["rescue", "event-end-move", "event-end-rescue"]:
            # create client and goal msg
            srv_proxy = rospy.ServiceProxy(action_server, srv)
            rospy.wait_for_service(action_server, timeout=20)
            request = req()
            ...
            print("NOT CALLING THE SERVICE ACTION NOT OF TYPE RESCUE")
